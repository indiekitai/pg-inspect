...
__export(index_exports, {
  ...
  InspectedCollation: () => InspectedCollation,
  InspectedConstraint: () => InspectedConstraint,
  InspectedDomain: () => InspectedDomain,
  InspectedEnum: () => InspectedEnum,
  InspectedExtension: () => InspectedExtension,
  InspectedFunction: () => InspectedFunction,
  InspectedIndex: () => InspectedIndex,
  InspectedPrivilege: () => InspectedPrivilege,
  InspectedRowPolicy: () => InspectedRowPolicy,
  InspectedSchema: () => InspectedSchema,
  InspectedSelectable: () => InspectedSelectable,
  InspectedSequence: () => InspectedSequence,
  InspectedTrigger: () => InspectedTrigger,
  InspectedType: () => InspectedType,
  PgInspector: () => PgInspector,
  inspect: () => inspect
});
...
function getSqlDir() {
  if (!_sqlDir) {
    try {
      ...
      _sqlDir = (0, import_path.join)(__dirname2, "..", "src", "sql");
      try {
        (0, import_fs.readFileSync)((0, import_path.join)(_sqlDir, "schemas.sql"));
      } catch {
        _sqlDir = (0, import_path.join)(__dirname2, "sql");
      }
    } catch {
      _sqlDir = (0, import_path.join)(__dirname, "sql");
    }
  }
  return _sqlDir;
}
function resourceText(subpath) {
  ...
  return (0, import_fs.readFileSync)((0, import_path.join)(sqlDir, filename), "utf-8");
}
